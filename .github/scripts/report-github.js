...
function info(item){
    let bulan = 0;
    try {
        const p = String(item.tanggal || "").includes("/") ? item.tanggal.split("/") : item.tanggal.split("-");
        const d = String(item.tanggal || "").includes("/") ? new Date(p[2], p[1]-1, p[0]) : new Date(p[0], p[1]-1, p[2]);
        const now = new Date();
        bulan = (now.getFullYear() - d.getFullYear()) * 12 + (now.getMonth() - d.getMonth());
...
async function main(){
    ...
    const mapItem = x => {
        ...
        return {
            ...
            waLink: hp.length > 2 ? `https://wa.me/${hp}?text=` + encodeURIComponent(waText) : "",
    ...
    const tanggalFormat = hariNama[now.getDay()] + ", " + now.getDate() + " " + bulanNama[now.getMonth()] + " " + now.getFullYear();
    ...
    const pesan = formatReport({
        ...
        saBulanIni: summary.totalSA || 0,
        saBulanLalu: summary.pointKurang || 0,
        targetKurang: Math.max((summary.pointKurang || 0) - (summary.totalSA || 0), 0),
        bonus: summary.totalBonus ? `Rp${Number(summary.totalBonus).toLocaleString("id-ID")}` : "-",
