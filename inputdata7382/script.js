...
async function executeSendComplaint(item, kendala) {
    ...
    const konfirmasiMsg = `Halo Selamat ${salamWaktu} Bpk/Ibu *${item.nama.trim().toUpperCase()}*.

Laporan kendala yang Bpk/Ibu sampaikan sudah kami teruskan kepada tim penanganan untuk dilakukan pengecekan lebih lanjut.

Apabila dalam waktu 1×24 jam kendala masih belum teratasi, silakan menghubungi saya kembali agar segera kami tindak lanjuti.

Terima kasih.`;
    const linkKonfirmasi = `https://api.whatsapp.com/send?phone=${getPureWaNumber(item.hp)}&text=${encodeURIComponent(konfirmasiMsg)}`;
...
📢 [KONFIRMASI KE PELANGGAN](${linkKonfirmasi})
📩 [KIRIM KE ADMIN](https://wa.me/?text=${encodeURIComponent(basePesan)})`;
    ...
    const form = new URL("https://docs.google.com/forms/d/e/1FAIpQLSfChAaigYgyWvhuYp_U3hECJriyjhsHhcT0--hxwNtV-W2E4Q/viewform");

    form.searchParams.set("usp", "pp_url");
    form.searchParams.set("entry.739802718", "Fery");
    form.searchParams.set("entry.1650728090", item.idCst || "");
    form.searchParams.set("entry.806689272", item.nama || "");
    form.searchParams.set("entry.136778382", getPureWaNumber(item.hp || ""));
    form.searchParams.set("entry.1436416031", item.alamat || "");
    form.searchParams.set("entry.1161812791", kendala || "");

    window.open(form.toString(), "_blank");
}
