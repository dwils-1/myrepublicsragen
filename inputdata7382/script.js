...
window.onload = () => {
    ...
    if (sessionStorage.getItem('isLoggedIn') === 'true') {
        ...
        const lockScr = document.getElementById('lockScreen');
        if (lockScr) lockScr.style.display = 'flex';
    }
...
async function executeWaAction(hp, id, nama, japo, paket, alamat, tgl, email, harga, withText, source, isPending, event) {
    ...
    const waBase = isMobile ? "https://wa.me/send" : "https://web.whatsapp.com/send";
...
function renderLeadsCards(data) {
    ...
    data.forEach(item => {
        ...
        const linkWA = "https://wa.me/send?phone=" + hpMurni + "&text=" + encodeURIComponent(rawPesanProspek);
        ...
        const linkWaIntro = "https://wa.me/send?phone=" + hpMurni + "&text=" + encodeURIComponent(rawPesanIntro);
...
Mohon bantuannya mas @admin`;
    ...
    const linkFollowUpCustomer = `https://wa.me/send?phone=${getPureWaNumber(item.hp)}&text=${encodeURIComponent(followUpCustomerMsg)}`;
...
🛠️ [RE-FOLLOW UP CS](${linkFollowUpCS})`;
    ...
    window.open(`https://wa.me/send?text=${encodeURIComponent(basePesan)}`, '_blank');
}
