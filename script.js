...
const CHAT_ID = '-1003594385102'; // ID Grup Supergroup yang sudah disesuaikan
const TOPIC_ID = '2'; // ID Topik MyRepublic
...
async function telegramFetch(text) {
    ...
    try {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({ 
                chat_id: CHAT_ID, 
                message_thread_id: TOPIC_ID, // Menambahkan pengiriman ke topik
                text: text, 
                parse_mode: 'HTML',
...
async function telegramSendPhoto(photoFile, caption) {
    const url = `https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`;
    const formData = new FormData();
    formData.append('chat_id', CHAT_ID);
    formData.append('message_thread_id', TOPIC_ID); // Menambahkan pengiriman ke topik
