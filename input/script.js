...
function renderCard(item, mode) {
    ...
    return `
    <div id="card-${item.idCst}" data-email="${item.email || '-'}" data-harga="${hT}" data-command="${(item.command || '')}" class="glass-card p-5 shadow-xl border-l-[12px] mb-4 ${cC}">
...
async function downloadImage(nama, id, paket, alamat, tgl, japo, hp, email) {
    document.getElementById('c-nama').innerText = nama; document.getElementById('c-id-bayar').innerText = cleanId(id);
    document.getElementById('c-paket').innerText = paket; document.getElementById('c-hp').innerText = formatBeautifulNumber(hp);
    document.getElementById('c-email').innerText = email; document.getElementById('c-alamat').innerText = alamat;
    document.getElementById('c-tgl').innerText = tgl || '-'; document.getElementById('c-japo').innerText = japo || '-';
    try {
        const el = document.getElementById('canvasTemplate');
        const can = await html2canvas(el, { scale: 2, useCORS: true, logging: false });
        const lnk = document.createElement('a'); lnk.download = `NOTIF_${cleanId(id)}.png`; lnk.href = can.toDataURL("image/png"); lnk.click();
        return true;
...
async function executeWaAction(hp, id, nama, japo, paket, alamat, tgl, email, harga, withText, source, isPending) {
    ...
    if (withText) {
        let txt = "";
        if (isPending) {
            // Pesan Khusus Follow-up Pemasangan dengan Promo & Rencana Kunjungan
            txt = `Halo Selamat ${slm} Yth. Bpk/Ibu *${nama.trim()}*,%0A%0ASaya dari *Dwi LS. MyRepublic Indonesia* ingin menindaklanjuti rencana pemasangan internet di alamat Bapak/Ibu.%0A%0AKami sedang ada *Promo Spesial* khusus untuk area Anda dan gratis biaya instalasi jika registrasi dilanjutkan hari ini.%0A%0ASaya akan melakukan kunjungan ke lokasi hari ini, apabila Bapak/Ibu berkenan untuk dipasang atau ingin konsultasi paket lebih lanjut bisa kabari saya ya. Terima kasih!`;
        } else if (source === 'qc') {
            txt = `Halo Selamat ${slm} Bapak/Ibu *${nama.trim()}*,%0ASaya Dwi MyRepublic, menanyakan kualitas internet di *${alamat}* apakah lancar? Terima kasih!`;
        } else {
            const day = now.getDate(); 
            let sel = day - parseInt(japo);
            if (sel >= -5 && sel <= 0) txt = `Halo Selamat ${slm} Yth. *${nama.trim()}*,%0ATagihan MyRepublic ID: *${cleanId(id)}* Jatuh tempo ${sel === 0 ? 'HARI INI' : Math.abs(sel)+' hari lagi'}. Mohon bayar tepat waktu ya.`;
            else if (sel > 0 && sel <= 7) txt = `Halo Selamat ${slm} Yth. *${nama.trim()}*,%0ATagihan MyRepublic ID: *${cleanId(id)}* sudah LEWAT jatuh tempo. Mohon segera dibayar ya.`;
            else txt = `Halo Selamat ${slm} Yth. *${nama.trim()}*,%0AMenanyakan layanan internet MyRepublic di *${alamat}* apakah ada kendala?`;
        }
        
        if (source !== 'qc') await downloadImage(nama, id, paket, alamat, tgl, japo, email);
        if (isProgress) fetch(`${scriptURL}?action=hapusHanyaTextProgress&idCst=${id}`);
        setTimeout(() => { window.location.href = `https://api.whatsapp.com/send?phone=${cH}&text=${txt}`; }, 1500);
    } else {
        if (isProgress) fetch(`${scriptURL}?action=hapusHanyaTextProgress&idCst=${id}`);
